...
#import "UIViewMGBadgeView.h"
#import <objc/runtime.h>

@implementation UIViewMGBadgeView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        _outlineWidth = 2.0;
        _minDiameter = 25.0;
        _position = MGBadgeCustomPositionTopRight;
        _displayIfZero = NO;
...
- (void)mg_updateBadgeViewPosition {
    ...
    switch (position) {
        ...
        case MGBadgeCustomPositionTopRight: {
            self.center = CGPointMake(57, 0);
            break;
        }
...
- (void)setBadgeView:(UIViewMGBadgeView *)badgeView {
    objc_setAssociatedObject(self, kMGBadgeViewPropertyKey, badgeView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIViewMGBadgeView *)badgeView {
    UIViewMGBadgeView *badgeView = objc_getAssociatedObject(self, kMGBadgeViewPropertyKey);
    if (!badgeView) {
        self.badgeView = [[UIViewMGBadgeView alloc] initWithFrame:CGRectZero];
        badgeView = objc_getAssociatedObject(self, kMGBadgeViewPropertyKey);
        [self addSubview:badgeView];
    }
    [badgeView mg_updateBadgeViewPosition];
    return badgeView;
}
