...
//fequalzro() from https://stackoverflow.com/a/1614761/184130
...
- (void)setScrollViewContentInset:(UIEdgeInsets)contentInset withBlock:(void (^)(BOOL finished))block;
- (void)setScrollViewContentInsetForLoadingWithBlock:(void (^)(BOOL finished))block;
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:NSLocalizedString(@"Halar para refrescar...",),
                                                       NSLocalizedString(@"Suelte para refrescar...",),
                                                       NSLocalizedString(@"Cargando...",),
                                                       nil];
        
        self.subtitles = [NSMutableArray arrayWithObjects:IGEmptyString, IGEmptyString, IGEmptyString, IGEmptyString, nil];
        self.viewForState = [NSMutableArray arrayWithObjects:IGEmptyString, IGEmptyString, IGEmptyString, IGEmptyString, nil];
    }
...
- (void)setScrollViewContentInsetForLoadingWithBlock:(void (^)(BOOL finished))block {
    ...
    [self setScrollViewContentInset:currentInsets withBlock:block];
}

- (void)setScrollViewContentInset:(UIEdgeInsets)contentInset {
    [self setScrollViewContentInset:contentInset withBlock:nil];
}
...
- (void)setState:(SVPullToRefreshState)newState {
    ...
    switch (newState) {
        ...
        case SVPullToRefreshStateLoading:{
            __weak typeof(self) weakSelf = self;
            [self setScrollViewContentInsetForLoadingWithBlock:^(BOOL finished) {
                if(previousState == SVPullToRefreshStateTriggered && weakSelf.pullToRefreshActionHandler)
                    weakSelf.pullToRefreshActionHandler();
            }];
...
- (void)rotateArrow:(float)degrees hide:(BOOL)hide {
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionAllowUserInteraction animations:^{
        weakSelf.arrow.layer.transform = CATransform3DMakeRotation(degrees, 0, 0, 1);
        weakSelf.arrow.layer.opacity = !hide;
