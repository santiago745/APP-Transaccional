...
- (void)customInit {
    ...
    _selectedTextFont = AP_HORIZONTAL_MENU_TEXT_FONT;
...
- (void)setSelectedTextFont:(UIFont *)textFont {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    for (int i=0; i< _visibleItems; i++) {
        customCell = (UITableViewCell *)[tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:i inSection:0]];
        if (customCell != nil) {
            [[customCell.contentView viewWithTag:1981]removeFromSuperview] ;
        }
        
    }
    
    UIImageView *imv = [[UIImageView alloc]initWithFrame:CGRectMake(40,40, 40, 10)];
    imv.tag = 1981;
    imv.image=[UIImage imageNamed:@"ico-position-hover-menu"];
    [selectedCell.contentView addSubview:imv];
    //selectedCell.textLabel.font = self.selectedTextFont;
}

-(void)tableView:(UITableView *)tableView didDeselectRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewCell *selectedCell = (UITableViewCell *)[tableView cellForRowAtIndexPath:indexPath];
    selectedCell.textLabel.font = self.textFont;
}
