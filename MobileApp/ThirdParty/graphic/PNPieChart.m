...
- (UILabel *)descriptionLabelForItemAtIndex:(NSUInteger)index{
    ...
    UILabel *descriptionLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 110, 80)];
    NSString *titleText = currentDataItem.textDescription;
    if(!titleText){
        titleText = [NSString stringWithFormat:@"%.1f%%",currentDataItem.value/ _total * 100];
    ...
    NSString *text = descriptionLabel.text;
    CGFloat heigth = 34;
    
    if (text.length > 18 && text.length <= 36) {
        heigth = 51;
    } else if (text.length > 36 && text.length <= 54) {
        heigth = 68;
    ...
    descriptionLabel.frame = CGRectMake(
                             ...
                             descriptionLabel.frame.size.width, heigth);
...
-(void)updateChartData:(NSArray *)data{
    _items = [NSArray arrayWithArray:data];
    [self strokeChart];
}
